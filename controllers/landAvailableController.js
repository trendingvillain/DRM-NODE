...
const createLandAvailable = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO land_available
      (name, area, place, varient, trees, amount, land_owner_id, status)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
      RETURNING *`,
      [name, area, place, varient, trees, amount, landOwnerId, status]
    );

    // Update land owner amount
    await pool.query(
...
const getAllLandAvailable = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM land_available WHERE status = 'active' ORDER BY id DESC"
    );
...
const getLandAvailableByLandOwnerId = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM land_available WHERE land_owner_id = $1',
      [landOwnerId]
    );
    ...
    if (result.rows.length > 0) {
      ...
      res.status(404).json({
        error: 'No land available records found for this land owner'
      });
    }
    ...
    console.error(error);
    res.status(500).json({
      error: 'Failed to retrieve land available by land owner ID'
    });
...
const updateLandAvailable = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      res.status(404).json({
        error: 'Land available record not found'
      });
    ...
    console.error(error);
    res.status(500).json({
      error: 'Failed to update land available'
    });
